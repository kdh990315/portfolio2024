export const projectDatas = [
	...
	},
	{
		...
		detail: {
			...
			4: {
				title: 'useContext 훅을 활용한 다크모드 활성화',
				reviews: [
			...
			5: {
				title: 'useContext 훅을 활용한 즐겨찾기 기능',
				reviews: [
					{1: '포켓몬카드의 별표시 및 포켓몬디테일의 별표시를 누르면 포켓몬을 가방에 넣을 수 있으며 즐겨찾기 버튼을 클릭하여 즐겨찾기한 포켓몬을 확인할 수 있도록 구현했습니다.'},
					{2: 'useContext Hook을 사용하여 중앙 집중적으로 상태를 관리할 수 있도록 하였으며, localStorage를 활용하여 새로고침 및 사용자가 브라우저를 떠나도 데이터가 유지되도록 구현했습니다.'},
				],
				video: '/video/pokemon/favorite.mp4',
			},
			6: {
				title: '코드 리펙토링',
				reviews: [
					{1: '각 컴포넌트에 흩어져있는 포켓몬 데이터를 useContext훅과 useReducer훅을 활용하여 중앙 집중식으로 관리하였습니다.'},
					{2: '흩어져있는 데이터들을 중앙에서 관리함으로써 컴포넌트가 깔끔해지고 상태의 일관성을 유지하기가 쉬워졌습니다.'},
				],
				video: '/',
			},
		},
		...
		<br><br>
		<p>
		7/21(( 즐겨찾기 기능 추가 및 코드 리펙토링 ))<br>
		코드 리펙토링을 진행하면서 가독성과 유지보수성으 크게 향상되었습니다. 여기저기 흩어져있던 데이터들을 중앙에서 관리하기 때문에 컴포넌트가 매우 꺠끗하게 관리될 수 있었습니다. <br>
		또한, 또한 상태의 일관성을 유지할 수 있었으며 코드의 재사용도 매우 수월했습니다.

		하지만, 이번 코드 리펙토링을 하면서 TypeScript의 중요성을 다시 한번 깨닫는 시간이 되었습니다. localStorage에 데이터를 추가하고 삭제할 때 string 형식과 number 형식의 차이로 인해 발생한 오류로 인해 많은 시간을 허비했습니다. TypeScript를 사용했다면, 이러한 형식 관련 오류를 컴파일 단계에서 미리 방지할 수 있었을 것입니다. 앞으로는 TypeScript를 적극적으로 도입하여 코드의 안정성과 가독성을 높이고, 형식 관련 문제를 사전에 예방할 계획입니다.
